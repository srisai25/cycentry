...
scan_png = os.path.join(current_dir, "assets\\images_proj\\Unknown-2.gif")
file_gif = os.path.join(current_dir, "assets\\images_proj\\file.gif")
logo_png = os.path.join(current_dir, "assets\\images_proj\\logo.png")

other_python_files_path = os.path.join(current_dir, 'other_python_files')
sys.path.append(other_python_files_path)

from assets.other_python_files.RunModel_1 import *
from assets.other_python_files.PE_file_extraction import *
from assets.other_python_files.combinatrix2a import *
from assets.other_python_files.wifi_detect import *
from assets.Portscan_final.runner import *
from assets.other_python_files.feedback import *
    ...
    def delete_virus(e):
        ...
        if virus!=[]:
            ...
            popup_continer_1.content=ft.Container(
                ft.Text("""Congratulations! Your system is now secure.\nCySentry has successfully detected and removed a potential threat from your computer.\nYour safety and privacy are our top priorities.\nIf you have any further concerns or questions, feel free to contact our support team.\nThank you for using CySentry to keep your system protected!
...
################################################ File Picker ################################################################### 
    ...
    def animate_popup_1():
        ...
        if(cl==[]):
            ...
            popup_continer_1.content=ft.Text("CONGRAGULATIONS, We didn't find any virus in this File")
    ...
    def file_detect(e):
        ...
        pridict=maha_predicto(file_loactions[0])
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    file_f_png = os.path.join(current_dir, "assets\\images_proj\\file_f.png")  
    ...
    file_upload= os.path.join(current_dir, "assets\\images_proj\\file_upload.png")  
    ...
    file_m_png = os.path.join(current_dir, "assets\\images_proj\\file.gif")
    #changes_done here
    ...
    file_text_png = os.path.join(current_dir, "assets\\images_proj\\file_text.png")
...
################################################ Thread 2 ##################################################################         
    ...
    def thread2():
        ...
        while(percentage!=100):
            ...
            num=ft.Text(percentage,color="#2F4858",size=30,weight=ft.FontWeight.W_900)
            pridict=predicto_1(file_paths[complete])
            ...
            scan_button.content= ft.Container(
                ft.Stack(controls=[
                        ...
                        ft.Container(ft.Row(controls=[ft.Text(" ",color="#2F4858",size=5),num,ft.Text("%",size=30,color="#2F4858",weight=ft.FontWeight.W_900)],alignment=ft.MainAxisAlignment.CENTER),alignment=ft.alignment.bottom_center,padding=ft.padding.only(bottom=93)),
            ...
            if pridict[0][0]==1:
                ...
                pridict2=maha_predicto(str(file_paths[complete]))
                if(pridict2[0]==1):
 ...
 #######################################################################################################################   
    def animate(e):
        ...
        scan_button.content=ft.Stack(
                        ...
                        ft.Container(
                            ft.Text("Please Wait",size=25,weight=ft.FontWeight.W_400,color="#2F4858"),alignment=ft.alignment.center,padding=40)
                        ])
    ...
    scan_1=ft.Container(ft.Stack(
        ...
        ft.Container(
            ft.Container(   
                ft.Image(src=scan_png,fit=ft.ImageFit.CONTAIN,color_blend_mode=ft.BlendMode.CLEAR),
                #scan inside color
                width=200,height=200,
                shape=ft.BoxShape.CIRCLE,border_radius=180),alignment=ft.alignment.center),
        ft.Container(
            ft.Text("SCAN",size=30,weight=ft.FontWeight.W_500,color="#2F4858"),alignment=ft.alignment.center,padding=70)
        ]),alignment=ft.alignment.center
    )
    ...
    scan_bg=ft.Container(
        ...
        rotate=ft.transform.Rotate(10, alignment=ft.alignment.center),
    ...
    def thread3():
        ...
        while(num):
            tan=0
            while(check_and_print_wifi_status()!="connected" and num==True):
            ...
            if(tan==1):
                popup_continer_3.content=ft.Text("Wi-Fi Connected Please Re-Analyze")
                popup_continer_3.height=50
                Newtwork_heading.height=0
    ...
    def network_check(e):
        ...
        global Analysing_condition
        time_1.error_text=None
        wifi_status=check_and_print_wifi_status()
        ...
        if(wifi_status=="connected"):
            ...
            network=network_run(time_1.value)
            global num
            num=False
            press_win_ctrl_t()
            if(network==1):
...
############    network_pridict=
        ...
        if(time_1.value!=""):
            try:
                ...
                if time_1.value=="":
                    ...
                    current_dir = os.path.dirname(os.path.abspath(__file__))
                    search_png = os.path.join(current_dir, "assets\\images_proj\\search.png")  
    ...
    network_heading_png=os.path.join(current_dir, "assets\\images_proj\\network.gif") 
    ...
    Newtwork_heading=ft.Container(ft.Row(controls=[Newtwork_heading_text,ft.Text("NetInspect",text_align="center",size=35,weight=ft.FontWeight.W_500)]),alignment=ft.alignment.center_left,width=1980,height=60,bgcolor="#5133c2")
    ...
    def url_detect(e):
        ...
        if(url==1):
            ...
            url_button.disabled=True
            page.update()
            # from assets.images_proj import feedback as feedback
            email_feedback(input.value,"somanathnayak1304@gmail.com")
            input.value=""
            page.update()
    ...
    input=ft.TextField(label="Enter your FeedBack Here",width=350,height=150,expand=True,border=False,on_change=feedback_change,multiline=True,min_lines=1,
    max_lines=3,border_color="#f7ff03",focused_border_color="#f7ff03",color="#f7ff03")     
    url_input=ft.Container(input,width=350,height=100,alignment=ft.alignment.center,margin=ft.margin.only(left=80,right=80),)
    ...
    feedback_png = os.path.join(current_dir, "assets\\images_proj\\feedback.png")
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    url2_png = os.path.join(current_dir, "assets\\images_proj\\feee.gif")  

    url_image2=ft.Container(ft.Image(src=url2_png,width=300,height=250,fit=ft.ImageFit.CONTAIN),alignment=ft.alignment.center,width=300,height=250,margin=ft.margin.only(left=120,top=20,right=120,bottom=20))
...
############################################## Title bar ########################################################################
    ...
    bgcolor="#25292f",
    leading_width=200,
    leading=ft.Container(ft.Image(src=logo_png,fit=ft.ImageFit.CONTAIN),width=200,height=100,alignment=ft.alignment.center,padding=ft.padding.only(top=1))
    )
############################################## Change Navigation ###############################################################    
