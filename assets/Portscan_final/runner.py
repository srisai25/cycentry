...
def network_run(duration_seconds):
    ...
    process = subprocess.Popen(["cmd", "/c", commands], shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,creationflags=subprocess.CREATE_NO_WINDOW)
    ...
    try:
        subprocess.run(tshark_command, shell=True, check=True, creationflags=subprocess.CREATE_NO_WINDOW)
